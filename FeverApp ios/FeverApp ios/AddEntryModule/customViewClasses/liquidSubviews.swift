...
enum DehydrationResponses: String {
    case no = "NO"
    case dryMucousMembranes = "YES_DRY_MUCOUS_MEMBRANES"
    case drySkin = "YES_DRY_SKIN"
    case tiredAppearance = "YES_TIRED_APPEARANCE"
    case sunkenEyeSockets = "YES_SUNKEN_EYE_SOCKETS"
    case fewerWetDiapers = "YES_FEWER_WET_DIAPERS"
    case sunkenFontanelle = "YES_SUNKEN_FONTANELLE"
    
    // Convert raw values to user-friendly descriptions
    var userFriendlyDescription: String {
        switch self {
        case .no:
            return "no signs of dehydration"
        case .dryMucousMembranes:
            return "dry mucous membranes"
        case .drySkin:
            return "dry and loose skin"
        case .tiredAppearance:
            return "tired appearance"
        case .sunkenEyeSockets:
            return "sunken eye sockets"
        case .fewerWetDiapers:
            return "fewer wet diapers"
        case .sunkenFontanelle:
            return "sunken fontanelle"
        }
    }
    ...
    static func fromTag(_ tag: Int) -> DehydrationResponses? {
        switch tag {
        case 0:
            return .no
        case 1:
            return .dryMucousMembranes
        case 2:
            return .drySkin
        case 3:
            return .tiredAppearance
        case 4:
            return .sunkenEyeSockets
        case 5:
            return .fewerWetDiapers
        case 6:
            return .sunkenFontanelle
        default:
            return nil
        }
    }
...
class liquidFirstViews: UIView{
    ...
    var dehydrationSigns: [String] = []
     ...
     func setupWarningSignsView() {
         ...
         WarningSignstopLabel.text = TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.QUESTION", defaultText: "Are there signs of dehydration?")
         ...
         let warningSigns = [TranslationsViewModel.shared.getTranslation(key: "PROFILE.ALERT.NO", defaultText: "No"), TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.2.LABEL", defaultText: "Yes, dry mucous membranes (=no tears when crying and drier lips than usual)"), TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.3.LABEL", defaultText: "Yes, the skin is dry and loose"),TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.4.LABEL", defaultText: "Yes, child looks very tired"),TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.5.LABEL", defaultText: "Yes, the eye sockets appear sunken"),TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.6.LABEL", defaultText: "Yes, significantly fewer wet diapers than normal"),TranslationsViewModel.shared.getTranslation(key: "DIARY.DRINK.DRINK.OPTION.7.LABEL", defaultText: "Yes, the fontanelle is sunken")
         ]
    ...
    func updateViewForEditingState() {
        guard let initialState = liquidModel.shared.initialLiquidState else {
            return // No initial state, meaning this is a new entry
        }
        
        guard let dehydrationSigns = initialState.dehydrationSigns else {
            return // No dehydration signs to process
        }
        ...
        for view in WarningSignsverticalStackView.arrangedSubviews {
            if let stackView = view as? UIStackView,
               let checkbox = stackView.arrangedSubviews.first as? UIButton {
                ...
                if let dehydrationResponse = DehydrationResponses.fromTag(checkbox.tag),
                   dehydrationSigns.contains(dehydrationResponse.rawValue) {
     ...
     @objc func confirmButtonTapped(){
         var checkedType = ""  // Initialize the checkedType string
            ...
            if selectedTags.contains(0) && selectedTags.count == 1 {
                checkedType = "no"
            }else if selectedTags.contains(1) || selectedTags.contains(2) || selectedTags.contains(3) || selectedTags.contains(4) || selectedTags.contains(5) || selectedTags.contains(6){
                checkedType = "yes"
            }
            //
        
         
         var dehydrationSignsEnumValues: [String] = []

         // Loop through dehydrationSigns and map descriptions to enum raw values
         for signTag in selectedTags {
             if let enumValue = DehydrationResponses.fromTag(signTag) {
                 dehydrationSignsEnumValues.append(enumValue.rawValue)
             } else {
               
             }
         }
     
         liquidModel.shared.saveDehydrationSigns(dehydrationSigns: dehydrationSignsEnumValues){isSaved in
             if isSaved{
                 // Call the closure with the determined checkedType
                 print("the following dehydration signs were saved \(dehydrationSignsEnumValues)")
...
class liquidDateTimeViews : UIView, UIScrollViewDelegate{
    ...
    var confirmButtonTap: ((_ _selctedDate : Date)->Void)?
    var noAnswerButtonTap: (()->Void)?
    var displayError: (()->Void)?
    @objc func noAnswerButtonTapped(){
        noAnswerButtonTap?()
    ...
    @objc func confirmButtonTapped(){
        ...
        liquidModel.shared.saveLiquidDateTime(liquidDate: selectedDate!){isSaved in
            if isSaved{
    ...
    override func layoutSubviews() {
        ...
        if liquidModel.shared.initialLiquidState != nil {
