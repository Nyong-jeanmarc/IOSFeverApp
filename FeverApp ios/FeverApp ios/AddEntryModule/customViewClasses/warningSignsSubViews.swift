...
import Foundation
import UIKit
enum WarningSignsResponses: String {
    case no = "NO"
    case touchSensitivity = "YES_TOUCH_SENSITIVITY"
    case shrillScreaming = "YES_SHRILL_SCREAMING_LIKE_I_VE_NEVER_HEARD_IT_BEFORE"
    case actingDifferently = "YES_ACTING_DIFFERENTLY_CLOUDED_CONSCIOUSNESS_APATHY"
    case seemsSeriouslySick = "YES_SEEMS_SERIOUSLY_SICK"
    
    // Convert raw values to user-friendly descriptions
    var userFriendlyDescription: String {
        switch self {
        case .no:
            return TranslationsViewModel.shared.getTranslation(key: "INFOS.BASIC_PRINCIPLES.TEXT.9", defaultText: "there are no warning signs,")
        case .touchSensitivity:
            return "touch sensitivity"
        case .shrillScreaming:
            return "shrill screaming like never heard before"
        case .actingDifferently:
            return "acting differently (clouded consciousness or apathy)"
        case .seemsSeriouslySick:
            return "seems seriously sick"
        }
    }
    
    // Get an enum case from its tag
    static func fromTag(_ tag: Int) -> WarningSignsResponses? {
        switch tag {
        case 0:
            return .no
        case 1:
        ...
        case 2:
            return .shrillScreaming
        case 3:
            return .actingDifferently
        case 4:
            return .seemsSeriouslySick
        default:
            return nil
        }
    }
...
class WarningSignsFirstSubview: UIView{
    ...
    var warningSigns: [String] = []
     ...
     func setupWarningSignsView() {
         ...
         WarningSignstopLabel.text = TranslationsViewModel.shared.getTranslation(key: "DIARY.WARNING_SIGNS.WARNING-SIGNS.QUESTION", defaultText: "Does {{name}} have other warning signs?").replacingOccurrences(of: "{{name}}", with: profileName!)
         ...
         let warningSigns = [TranslationsViewModel.shared.getTranslation(key: "PROFILE.ALERT.NO", defaultText: "No"), TranslationsViewModel.shared.getTranslation(key: "DIARY.WARNING_SIGNS.WARNING-SIGNS.OPTION.2.LABEL", defaultText: "Yes, touch sensitivity"), TranslationsViewModel.shared.getTranslation(key: "DIARY.WARNING_SIGNS.WARNING-SIGNS.OPTION.3.LABEL", defaultText: "Yes, shrill screaming like I've never heard it before"), TranslationsViewModel.shared.getTranslation(key: "DIARY.WARNING_SIGNS.WARNING-SIGNS.OPTION.4.LABEL", defaultText: "Yes, acting differently, clouded consciousness, apathy"), TranslationsViewModel.shared.getTranslation(key: "DIARY.WARNING_SIGNS.WARNING-SIGNS.OPTION.5.LABEL", defaultText: "Yes, seems seriously sick")
         ]
    ...
    var displayError: (()->Void)?
     ...
     @objc func confirmButtonTapped(){
         ...
         var warningSignsEnumValues: [String] = []

         for tag in selectedTags {
             if let enumCase = WarningSignsResponses.fromTag(tag) {
                 warningSignsEnumValues.append(enumCase.rawValue)
             } else {
               
             }
         }
         
         warningSignsModel.shared.saveWarningSigns(warningSigns: warningSignsEnumValues){isSaved in
             if isSaved{
                 print(warningSignsEnumValues)
    ...
    func updateWarningSignsViewForEditingState() {
        // Ensure we have the initial state
        guard let initialState = warningSignsModel.shared.initialWarningSigns else {
            print("No initial warning signs state available.")
            return
        }
        
        // Extract the signs array
        guard let signs = initialState.signs else {
            print("No signs available in the initial state.")
            return
        }
        ...
        for view in WarningSignsverticalStackView.arrangedSubviews {
            if let stackView = view as? UIStackView,
               let checkbox = stackView.arrangedSubviews.first as? UIButton {
                ...
                // Get the tag and map it to a WarningSignsResponses enum
                if let warningSignResponse = WarningSignsResponses.fromTag(checkbox.tag),
                   signs.contains(warningSignResponse.rawValue) {
                    checkbox.isSelected = true
                }
            }
        }
        
        // Enable and update the confirm button state
     ...
     override init(frame: CGRect) {
         ...
         updateWarningSignsViewForEditingState()
     }
...
class WarningSignsDateTimeView: UIView, UIScrollViewDelegate{
    ...
    @objc func confirmButtonTapped(){
        ...
        warningSignsModel.shared.saveWarningDate(date: selectedDate!){isSaved in
            if isSaved {
    ...
    func scrollToInitialState() {
        ...
        guard let initialState = warningSignsModel.shared.initialWarningSigns,
    ...
    override func layoutSubviews() {
        ...
        if warningSignsModel.shared.initialWarningSigns != nil {
