...
class SymptomsFirstSubview: UIView{
    ...
    enum Symptom: String {
        case no = "NO"
        case cough = "COUGH"
        case constrainedBreathing = "CONSTRAINED_BREATHING"
        case freezingChills = "FREEZING_CHILLS"
        case fatigueExhaustion = "FATIGUE_EXHAUSTION"
        case senseOfSmellTasteDisturbed = "SENSE_OF_SMELL_TASTE_DISTURBED"
        case mucus = "MUCUS"
        case tonsillitis = "TONSILLITIS"
        ...
        var userFriendlyDescription: String {
            ...
            case .cough:
                return "Cough"
            case .constrainedBreathing:
                return "Constrained breathing"
            case .freezingChills:
                return "Freezing/Chills"
            case .fatigueExhaustion:
                return "Fatigue, exhaustion"
            case .senseOfSmellTasteDisturbed:
                return "Sense of smell/taste disturbed"
            case .mucus:
                return "Mucus (ear, nose, and throat)"
            case .tonsillitis:
                return "Tonsillitis"
            case .jointSwelling:
                return "Joint swelling"
            case .teething:
                return "Teething"
            case .other:
        ...
        static func fromTag(_ tag: Int) -> Symptom? {
            switch tag {
            case 0:
                return .no
            case 1:
                return .cough
            case 2:
                return .constrainedBreathing
            case 3:
                return .freezingChills
            case 4:
                return .fatigueExhaustion
            case 5:
                return .senseOfSmellTasteDisturbed
            case 6:
                return .mucus
            case 7:
                return .tonsillitis
     ...
     func setupWarningSignsView() {
         ...
         WarningSignstopLabel.text = TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.QUESTION", defaultText: "Does {{name}} have symptoms?").replacingOccurrences(of: "{{name}}" , with: profileName!)
         ...
         let warningSigns = [TranslationsViewModel.shared.getTranslation(key: "PROFILE.ALERT.NO", defaultText: "No"),TranslationsViewModel.shared.getTranslation(key: "DIARY.DOCTOR.DOCTOR_DIAGNOSIS-2.OPTION.10.LABEL", defaultText: "Cough"), TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.3.LABEL", defaultText: "Constrained breathing"), TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.4.LABEL", defaultText: "Freezing / chills"), TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.5.LABEL", defaultText: "Fatigue, exhaustion"),TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.6.LABEL", defaultText: "Sense of smell/taste disturbed")
                             ,TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.7.LABEL", defaultText: "Mucus (ear, nose and throat)"),TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.8.LABEL", defaultText: "Tonsillitis"),TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.9.LABEL", defaultText: "Joint swelling"),TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.11", defaultText: "Teething"),TranslationsViewModel.shared.getTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OPTION.10.LABEL", defaultText: "Other symptoms")
         ]
     ...
     @objc func confirmButtonTapped(){
         var checkedType = ""  // Initialize the checkedType string
            ...
            if selectedTags.contains(0) && selectedTags.count == 1 {
                checkedType = "no"
            } else if selectedTags.contains(10) || (selectedTags.contains(6) && selectedTags.count > 1) {
                checkedType = "others"
            } else if selectedTags.contains(1) || selectedTags.contains(2) || selectedTags.contains(3) || selectedTags.contains(4) || selectedTags.contains(5) || selectedTags.contains(6) || selectedTags.contains(7) || selectedTags.contains(8) || selectedTags.contains(9){
                checkedType = "yes"
            }
         
         var symptomsEnumValues: [String] = []

         for symptomTag in selectedTags {
             if let enumCase = Symptom.fromTag(symptomTag) {
                 symptomsEnumValues.append(enumCase.rawValue)
             } else {
                
             }
         }
         symptomsModel.shared.saveSymptoms(symptoms: symptomsEnumValues){isSaved in
             if isSaved{
                 print("saved symptoms are \(symptomsEnumValues)")
    ...
    func updateViewForEditingState() {
        guard let initialState = symptomsModel.shared.initialSymptomsState else {
            return // No initial state, meaning this is a new entry
        }

        guard let symptoms = initialState.symptoms else {
            return // No symptoms to process
        }
        ...
        for view in WarningSignsverticalStackView.arrangedSubviews {
            if let stackView = view as? UIStackView,
               let checkbox = stackView.arrangedSubviews.first as? UIButton {
                ...
                // Get the tag and check if it corresponds to any symptom in the initial state
                if let symptom = Symptom.fromTag(checkbox.tag),
                   symptoms.contains(symptom.rawValue) {
...
class otherSymptompsTextFieldView: UIView, UITextFieldDelegate {
    ...
    func checkForInitialState(){
        ...
        guard let initialState = symptomsModel.shared.initialSymptomsState,
              let otherSymptoms = initialState.otherSymptoms else {
            print("NO INITIAL STATE FOR TEMP")
            return
        }
        painTextField.text = otherSymptoms.joined(separator: ", ")
        // Update the confirm button state
    ...
    private func setupView() {
        ...
        // Question Label
        questionLabel.text = TranslationsViewModel.shared.getAdditionalTranslation(key: "DIARY.SYMPTOMS.SYMPTOMS.OTHER_SYMPTOMS.LABEL", defaultText: "Which other symptoms does {{name}} have").replacingOccurrences(of: "{{name}}", with: profileName!)
    ...
    private func setupConstraints() {
      ...
      noAnswerButton.addTarget(self, action: #selector(noButtonTapped), for: .touchUpInside)
        nextButton.addTarget(self, action: #selector(confirmButtonTapped), for: .touchUpInside)
    ...
    @objc func confirmButtonTapped(){
        ...
        otherSymptoms.append(text)
        symptomsModel.shared.saveOtherSymptoms(otherSymptoms: self.otherSymptoms){isSaved in
            if isSaved {
                print("other symptoms \(text) saved")
                self.confirmButtonTap?()
            }else{
                self.displayError?()
            }
...
class SymptomsDateTimeView: UIView, UIScrollViewDelegate{
    ...
    @objc func confirmButtonTapped(){
        ...
        symptomsModel.shared.saveSymtomsDate(date: selectedDate!){isSaved in
            if isSaved {
